...
;(function () {
  ...
  // Letakkan fungsi ini di dalam file token.js Anda, misalnya setelah fungsi getFromLocalStorage

function showCustomAlert(message, type = 'error', duration = 5000) {
  // Hapus notifikasi yang mungkin sudah ada untuk menghindari tumpukan
  const existingAlert = document.getElementById('custom-alert-mentari');
  if (existingAlert) {
    existingAlert.remove();
  }

  // Buat elemen utama notifikasi
  const alertElement = document.createElement('div');
  alertElement.id = 'custom-alert-mentari';
  alertElement.className = `custom-alert-mentari ${type}`;

  // Siapkan ikon SVG berdasarkan tipe notifikasi (error atau success)
  const icons = {
    error: `
  ...
  // Atur waktu untuk menghapus notifikasi secara otomatis
